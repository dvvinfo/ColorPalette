  ...
  <div class="container mx-auto px-4 py-8">
    <div class="bg-card-bg rounded-lg p-6">
      <div class="flex items-center gap-4 mb-6">
        <BackButton />
        <h1 class="text-2xl font-bold">{{ $t('navigation.wheel') }}</h1>
      </div>
...
const wheelIframe = ref<HTMLIFrameElement | null>(null)
const createdGameId = ref<number | null>(null)

const gameUrl = computed(
  () => `${window.location.origin}/games/project-1-wheel-of-fortune-master/index.html`,
)
...
async function initializeGame() {
  ...
  try {
    ...
    if (gameId) {
      ...
      console.error(t('games.wheel.debug.gameIdNotFound'))
    }
  } catch (e) {
...
async function onPlay() {
  ...
  if (!isNaN(num) && num >= 1 && num <= (authStore.user?.balance || 0) && createdGameId.value) {
    try {
      ...
      if (wheelIframe.value && wheelIframe.value.contentWindow) {
        wheelIframe.value.contentWindow.postMessage({ type: 'PLAY_WHEEL', bet: num }, '*')
      }
      bet.value = ''
    } catch (error) {
...
onMounted(async () => {
  window.addEventListener('message', handleWheelResult)
  await initializeGame()
})

onUnmounted(() => {
  window.removeEventListener('message', handleWheelResult)
})

function handleWheelResult(event: MessageEvent) {
  if (event.data && event.data.type === 'WHEEL_RESULT') {
