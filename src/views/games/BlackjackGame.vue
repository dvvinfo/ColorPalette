  ...
  <div class="container mx-auto px-4 py-8">
    <div class="bg-card-bg rounded-lg p-6">
      <div class="flex items-center gap-4 mb-6">
        <BackButton />
        <h1 class="text-2xl font-bold">Блэкджек</h1>
      </div>
      ...
      <div class="aspect-video bg-black/20 rounded-lg flex items-center justify-center mb-6">
        <iframe
          ref="gameFrame"
          src="https://www.jacquelynmarcella.com/blackjack/"
...
async function onPlay() {
  ...
  if (!isNaN(num) && num >= 1 && num <= (authStore.user?.balance || 0)) {
    try {
      // Используем ID 7 для блэкджека (из моковых данных)
      await gamesStore.playGame(7, num)
...
onMounted(async () => {
  ...
  await gamesStore.fetchGameById(7)
})
</script>
