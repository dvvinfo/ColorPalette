      ...
      <div v-if="availableBonuses.length > 0" class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="bonus in availableBonuses" :key="bonus.id" class="bg-card-bg rounded-lg p-4">
            ...
            <div v-if="bonus.promo_code && !bonus.isActivated" class="space-y-3">
              ...
              <div v-if="bonus.hasInvalidPromo" class="text-red-500 text-xs">
            ...
            <BaseButton
              v-else-if="!bonus.promo_code && !bonus.isActivated"
              size="md"
              class="w-full"
              @click="activateBonus(bonus)"
            >
...
const error = ref('')
const bonuses = ref<(Bonus & { isActivated?: boolean; hasInvalidPromo?: boolean })[]>([])
const promoInputs = reactive<Record<number, string>>({})

// Разделяем бонусы на активированные и доступные
const activeBonuses = computed(() => bonuses.value.filter((bonus) => bonus.isActivated))

const availableBonuses = computed(() =>
  bonuses.value.filter((bonus) => !bonus.isActivated && bonus.is_active),
)
...
const loadBonuses = async () => {
  try {
    ...
    bonuses.value = response.data.map((bonus) => ({
      ...bonus,
      isActivated: false, // В реальном приложении это должно приходить с сервера
...
const activateBonus = async (
  bonus: Bonus & { isActivated?: boolean; hasInvalidPromo?: boolean },
) => {
  try {
    if (bonus.promo_code) {
      ...
      // Обычная активация бонуса (если такая логика есть)
      bonus.isActivated = true
